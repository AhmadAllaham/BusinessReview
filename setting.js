(async function () {
  ...
  function selectedValues(targetId) {
    return [...document.querySelectorAll(`#${targetId} input:checked`)]
  ...
  function syncUserActivationButton(user) {
    ...
    if (activeCheckbox) activeCheckbox.disabled = !user || isSelf;
    if (roleSelect) roleSelect.disabled = !user || isSelf;
  }

  async function loadCountries() {
    const snapshot = await db.collection('system').doc('countries').get();
    countries = snapshot.exists
      ? [...new Set(snapshot.data().values || [])].sort((left, right) => left.localeCompare(right))
      : [];
